...
- **UI 重构&优化**：顶部 header 替换为启动/重启按钮，支持一键启停服务
- **配置页增强**：新增添加/删除模型功能，支持动态增删后端 API 配置
- **配置同步**：新增"从文件重新加载"按钮，直接修改 yaml 后可一键同步到 UI

### 2026-04-29

-- **新增UI界面**：用户可以通过界面配置代理规则、等。支持后台等。支持后台到托盘运行，无窗口模式。
-- **新增证书自动生成功能**：在启动时检查证书文件是否存在，若不存在则自动生成。

### 2026-04-25

Trae 给自定义模型默认开启了视觉支持，部分模型实际不支持，为了提高用户体验，新增了视觉 fallback 机制。
- **新增模型视觉支持属性**：为每个 API 模型增加 `supports_image` 配置，可标识模型是否支持视觉输入
- **新增视觉 fallback 机制**：当请求的模型不支持视觉但消息中包含图片时，自动调用 vision fallback 模型（可配置， 默认是第一个支持视觉的模型）对图片进行描述，并将描述文本插入原消息流中；
